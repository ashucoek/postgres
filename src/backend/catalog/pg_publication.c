	...
	foreach(cell, pubschemalist)
	{
		...
		if (except_relids != NIL)
		{
			...
			foreach(rlc, schemaRels)
			{
				 ...
				 *
				 * XXX: this ancestor check uses the publication-wide
				 * except_relids and does not consider which schema the EXCEPT
				 * was bound to.  When a publication has multiple schemas
				 * (e.g. FOR TABLES IN SCHEMA s1, TABLES IN SCHEMA s2 EXCEPT
				 * (TABLE s2.parent)) and a partition of the EXCEPT'd parent
				 * lives in a DIFFERENT schema (s1) that is itself
				 * unconditionally included, we treat that partition as
				 * excluded because its ancestor matches the global
				 * except_relids. Need to decide the expected behavior here.
				 */
