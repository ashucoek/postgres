...
GetAllPublicationRelations(Publication *pub, char relkind)
{
	...
	if (relkind == RELKIND_RELATION)
		exceptlist = get_publication_relations(pubid, pubviaroot ?
											   PUBLICATION_PART_ALL :
											   PUBLICATION_PART_LEAF, true);
...
bool
is_relation_excluded(Oid relid, Oid pubid)
{
	HeapTuple	tup;
	bool		result = false;

	tup = SearchSysCache2(PUBLICATIONRELMAP, ObjectIdGetDatum(relid), ObjectIdGetDatum(pubid));
	if (HeapTupleIsValid(tup))
	{
 ...
 */
bool
publication_has_any_exception(Oid pubid)
{
	Relation	pubRel;
	ScanKeyData skey;
	SysScanDesc scan;
	...
	/* Open pg_publication_rel with AccessShareLock */
	pubRel = table_open(PublicationRelRelationId, AccessShareLock);

	/*
	 * Look up by publication OID. Using the index on (prpubid) is highly
	 * efficient.
	 */
	ScanKeyInit(&skey,
				Anum_pg_publication_rel_prpubid,
	...
	/* We only need to find the first occurrence of prexcept = true */
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		Form_pg_publication_rel pubrelform = (Form_pg_publication_rel) GETSTRUCT(tup);

		if (pubrelform->prexcept)
		{
			found = true;
			break;
		}
 ...
 * is_schema_published
 *
 * Checks if the given namespace OID is part of the publication's
 * schema list. This corresponds to the 'FOR TABLES IN SCHEMA' syntax.
 */
static bool
is_schema_published(Oid nspid, Oid pubid)
{
	/*
	 * Search the syscache for pg_publication_namespace using the (nspid,
	 * pubid) index.
	 */
...
/*
 * is_relation_or_ancestor_excluded
 *
 ...
 */
static bool
is_relation_or_ancestor_excluded(Oid relid, Oid pubid)
{
	List	   *ancestors;
	ListCell   *lc;
	...
	if (is_relation_excluded(relid, pubid))
		return true;
	...
	foreach(lc, ancestors)
	{
		...
		if (is_relation_excluded(ancestor, pubid))
		{
			in_except = true;
			break;
		}
 ...
 *
 * Given a root relation and a list of publications, calculate the set of
 * relations that are effectively published. This is necessary for
 * "FOR ALL TABLES" publications that use "EXCEPT TABLE" filters.
 *
 * The function resolves the smallest possible set of ancestors that cover
 * all non-excluded leaf partitions.
 */
Datum
pg_get_publication_effective_tables(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		 ...
		 * clause, or if none of the publications define exceptions, the root
		 * relation alone is sufficient as the result.
		 */
		...
		else
		{
			...
			foreach_oid(curr_relid, all_leaves)
			{
				 ...
				 * publication set.
				 */
				if (is_relation_or_ancestor_excluded(curr_relid, except_pub_id))
					except_leaves = lappend_oid(except_leaves, curr_relid);
				else
			 ...
			 */
			foreach_oid(curr_relid, except_leaves)
			{
		...
		TupleDescInitEntry(tupdesc, (AttrNumber) 1, "schemaname",
						   TEXTOID, -1, 0);
