...
/*
 * Ensure that at most one publication in the given list defines an
 * EXCEPT TABLE clause.
 */
void
CheckPublicationsForExceptClauses(List *except_publications)
{
	StringInfoData pubnames;

	/* Nothing to do if zero or one publication has EXCEPT. */
	...
	list_free_deep(except_publications);
...
check_publications_except_list(WalReceiverConn *wrconn, List *publications)
{
	...
	CheckPublicationsForExceptClauses(except_publications);
}
